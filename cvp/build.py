...
Requires the VitalDB cases downloaded locally and two inputs you provide:
  ...
  - the VitalDB clinical table (height, ASA) and the per-case CVP label.
...
CLINICAL = "/path/to/clinical.csv"       # columns: caseid,height,asa,cvp_numeric
...
def main():
    ...
    with open(OUT, "w", newline="") as f:
        w = csv.writer(f); w.writerow(["caseid"] + FEATURES + ["height", "asa", "cvp_numeric", "elevated"])
        for cid, starts in windows.items():
            ...
            w.writerow([cid] + [round(med[fe], 5) for fe in FEATURES] +
                       [c.get("height", ""), c.get("asa", ""), round(cvp, 2), int(cvp > 12)])
    print(f"wrote {OUT}")
